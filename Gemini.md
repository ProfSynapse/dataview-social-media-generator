...
// Function to call Gemini API and generate content based on the platform
...
${content}`;
    ...
    try {
        console.log(`Sending request to Gemini for ${platform}`);
        const response = await fetch("https://api.gemini.ai/v1/chat/completions", { // Replace with actual Gemini API endpoint
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${GEMINI_API_KEY}` // Include if required by Gemini
            },
            body: JSON.stringify({
                model: "gemini-pro", // Adjust model if necessary
                messages: [
